...
There are `r nrow(nobel_living)` rows and `r ncol(nobel_living)` columns in the nobel dataset. Each row represents a nobel winner.
...
ggplot(data = nobel_living_science, aes(y = country_us)) +
  ...
  facet_wrap(~category)
```
There appears to be more people in the US than outside of the US in each category. 
...
In general, our visualization does support Buzzfeed's claim because most nobel prize winners were born outside the US but living in the US when they won the award. 
